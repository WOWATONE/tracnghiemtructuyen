...
namespace TNTT.Class
{
    class C_Lop
    {
        ...
        {
            string sql = "SELECT idlop,tenlop,malop,makhoa,khoa_idkhoa FROM LOP L, KHOA K WHERE L.khoa_idkhoa = K.idkhoa";
            DataTable dt = new DataTable();
            dt = db.GetData(sql);
            return dt;
        }

        public DataTable GetListById(string idkhoa)
        {
            string sql = "SELECT * FROM LOP WHERE khoa_idkhoa = " + idkhoa;
            DataTable dt = new DataTable();
            dt = db.GetData(sql);
            return dt;
        }
